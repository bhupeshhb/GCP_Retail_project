...
CREATE TABLE IF NOT EXISTS `retail-data-lake.gold_dataset.sales_summary`
AS
SELECT 
    o.order_date,
    p.category_id,
...
FROM `retail-data-lake.silver_dataset.orders` o
JOIN `retail-data-lake.silver_dataset.order_items` oi ON o.order_id = oi.order_id
...
CREATE TABLE IF NOT EXISTS `retail-data-lake.gold_dataset.customer_engagement`
AS
SELECT 
    c.customer_id,
...
FROM `retail-data-lake.silver_dataset.customers` c
LEFT JOIN `retail-data-lake.silver_dataset.orders` o ON c.customer_id = o.customer_id
...
CREATE TABLE IF NOT EXISTS `retail-data-lake.gold_dataset.product_performance`
AS
SELECT 
    p.product_id,
    p.name AS product_name,
...
FROM `retail-data-lake.silver_dataset.products` p
LEFT JOIN `retail-data-lake.silver_dataset.categories` c ON p.category_id = c.category_id
LEFT JOIN `retail-data-lake.silver_dataset.product_suppliers` ps ON p.product_id = ps.product_id
LEFT JOIN `retail-data-lake.silver_dataset.suppliers` s ON ps.supplier_id = s.supplier_id
LEFT JOIN `retail-data-lake.silver_dataset.order_items` oi ON p.product_id = oi.product_id
...
SELECT 
    ...
    SUM(oi.quantity) AS total_units_sold,
    SUM(oi.price * oi.quantity) AS total_revenue
FROM `retail-data-lake.silver_dataset.suppliers` s
LEFT JOIN `retail-data-lake.silver_dataset.product_suppliers` ps ON s.supplier_id = ps.supplier_id
LEFT JOIN `retail-data-lake.silver_dataset.order_items` oi ON ps.product_id = oi.product_id
